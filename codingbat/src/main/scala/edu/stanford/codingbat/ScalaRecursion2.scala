...
def groupNoAdj(start:Int, nums:Array[Int], target:Int):Boolean = ???
...
def groupSumClump(start:Int, nums:Array[Int], target:Int):Boolean = ???
...
def splitOdd10(nums:Array[Int]):Boolean = ???
...
//split53(= ???1, 1, 1) → false
//split53(= ???2, 4, 2) → true
def split53(nums:Array[Int]):Boolean = ???
