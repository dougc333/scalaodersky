...
//note to self: good b/c this is harder for immutable arrays
...
def fix34(nums:Array[Int]):Array[Int] = {
  for(i<-0 to nums.size){
    if(nums(i)==3){
      nums(nums.slice(i,nums.size).indexOf(4))=nums(i+1)
      nums(i+1)=4
    }
  }
  nums
...
def  fix45(nums:Array[Int]):Array[Int] = {
  for(i<-0 to nums.size){
    if(nums(i)==4){
      nums(nums.slice(i,nums.size).indexOf(5))=nums(i+1)
      nums(i+1)=5
    }
  }
  nums
...
//NOTE TO SELF: not scalish style....
def  canBalance(nums:Array[Int]):Boolean = {
  var returnVal=false
  for(i<-0 to nums.length){
   val sumRight = nums.slice(0,i).sum
   val sumLeft = nums.slice(i,nums.size).sum
   if(sumRight==sumLeft){
     returnVal=true
   }
  }
...
//NOTE TO SELF: possible to compose anon functions one for each element in array?
def linearIn(outer:Array[Int], inner:Array[Int]):Boolean = {
  var returnVal=true
  for(i<-0 to inner.size){
    if(!outer.contains(inner(i))) returnVal=false
  }
  returnVal
...
//(the = ???1, 2, 3 part). Return the size of the largest mirror section found in the given 
//array. 

///maxMirror(= ???1, 2, 3, 8, 9, 3, 2, 1) → 3
//maxMirror(= ???1, 2, 1, 4) → 3
//maxMirror(= ???7, 1, 2, 9, 7, 2, 1) → 2
def  maxMirror(nums:Array[Int]):Int = ???
