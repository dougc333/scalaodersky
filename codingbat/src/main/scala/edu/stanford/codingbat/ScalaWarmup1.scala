...
def parrotTrouble(talking:Boolean, hour:Int):Boolean ={
  if((talking && (hour<7)) || (talking && (hour>20))) true
  else 
    false
}
...
def  nearHundred(n:Int):Boolean= {
  if(Math.abs(n-100)<=10 || Math.abs(n-200)<=10) true
  else
   false
}
...
def frontBack(str:String):String= {
  if(str.length>1)
     str.last.toString++str.substring(1,str.length-1)++str.head.toString
  else 
    str
}
...
def front3(str:String):String= {
  if(str.length<3)
    ...
    str.take(3)+str.take(3)+str.take(3);
}
...
def backAround(str:String):String= {
  str.charAt(str.length()-1)+str+str.charAt(str.length()-1);
}
...
def front22(str:String):String= {
  if(str.length()>=2){
	  return str.take(2)+str+str.take(2);
  }
  return str;
}
...
def close10(a:Int, b:Int):Int= {
  val list = List(Math.abs(a-10),Math.abs(b-10))
  if(list(0)==list(1)) 0
  else if(list(0)<list(1)) a
  else b
}
...
def endUp(str:String):String= {
  str.dropRight(3)+str.takeRight(3).toUpperCase
} //note to self: document dont to do exception testing for bounds like java
