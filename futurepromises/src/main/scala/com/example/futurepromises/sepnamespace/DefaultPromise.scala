...
object DefaultPromise{
  def apply[T]() = new DefaultPromise[T]()
  def success[T](value:T):DefaultPromise[T]={
    val promise = DefaultPromise[T]()
...
class DefaultPromise[T]{
  ...
  def value: Try[T] = {
    if(this.isCompleted){
      ...
      throw new IllegalStateException("promise not completed yet")
    }
  }
  ...
  def tryComplete(result:Try[T]):Boolean = {
    if(result==null){
      throw new IllegalStateException("result cant be null")
    }
    synchronized{
     if(isCompleted){
       false
